...
const { attentionembed } = require("../../../utils/attentionembed");
  ...
  async run(client, message, args) {
    ...
    try {
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("#EA8C8F")
    ...
    if (serverQueue) {
      ...
      const newsong = new MessageEmbed()
        ...
        .setColor("#EA8C8F")
        .setURL(song.url)
        .setDescription(`Song has been added to queue successfully!`)
    ...
    try {
      ...
      return attentionembed(message, `I could not join the channel due to: ${error}`);
    }
  }
};
